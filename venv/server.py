...
from ipconfig import change_ip
...
async def echo(websocket, path):
    ...
    async for message in websocket:
        ...
        #Coleta as variáveis e envia para o client        
            ...
            for item in var:
                tags.append(readMultiple(item, user_states.get("ip_clp"), user_states.get("slotCLP")))
        ...
        #Cadastro de novo usuário
            ...
            if user is None:
                ...
                dados = {"Usuario": data.get("usuario"),
        ...
        #Login de usuário
            ...
            if user is not None:
                ...
                if bcrypt.checkpw(password_entry, password_fromdb):
                    ...
                    user_states["slotCLP"] = user.get("slotCLP")
        ...
        #Altera configurações de IP do CLP
            ...
            data_update = {
                "$set": {
                    "_ipCLP": data.get("_ip"),
                    "slotCLP": data.get("slotCLP")
                }
            }
            result = dbUpdateOne(documento_id, data_update, "user")
            print("IP Modificado!")
            print(result)
            user_states["ip_clp"] = data.get("_ip")
            user_states["slotCLP"] = data.get("slotCLP")
            response = {"flag": "clpUpdated"}
            await websocket.send(json.dumps(response))

        #Altera configurações de IP do Dispositivo
            ...
            result = dbUpdateOne(documento_id, data_update, "user")
            print("IP Modificado!")
            change_ip(data.get("_ip"))
            print(result)
            user_states["ip_disp"] = data.get("_ip")
        ...
        #Carrega configurações de IP do usuário
            ...
            response["slotCLP"] = result.get("slotCLP")
        ...
        #Teste de variável
            ...
            result = readTag(data.get("name"), user_states.get("ip_clp"), user_states.get("slotCLP"))
            print(result.Status)
...
start_server = websockets.serve(echo, ip_address, 5000)
